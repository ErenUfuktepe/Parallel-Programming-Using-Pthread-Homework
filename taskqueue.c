...
int main(int argc, char* argv[]) {
   ...
   if(strtol(argv[1], NULL, 10) == 0 || strtol(argv[2], NULL, 10) == 0)
   {
   ...
   n = strtol(argv[2], NULL, 10); //Number of task
   
   /* Allocate array for threads */
   pthread_t* thread_handles = malloc(sizeof(pthread_t)*thread_count);
   pthread_attr_t *attr = malloc(sizeof(pthread_attr_t)*thread_count);   
   ...
   for(int i = 0 ; i < thread_count ; i++)
   {
   	...
   	if(pthread_create(&thread_handles[i],&attr[i],Thread_work,(void *)thread)==0)
   		my_thread++;
   	else{
   ...
   for(int i = 0 ; i < thread_count ; i++)///dönmüyor
   {
...
void Task_queue(int n) {
  ...
  /*Some synch code here!!!*/
   ...
   while(1)
   {
   	if(thread_count == my_thread && tasks_head == NULL){
...
{	
	...
	{	
	        ...
	        Task_dequeue(my_rank, which_task_p, option_p, data_p);
		pthread_cond_wait(&cond_task, &queue_mutex); 
